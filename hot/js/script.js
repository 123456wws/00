...
document.addEventListener("DOMContentLoaded", function() {
    console.log("DOMContentLoaded - 开始加载页面");
    ...
    document.querySelectorAll('img').forEach(img => {
        console.log("检查图片:", img.src);
        if (img.getAttribute('data-src')) {
            console.log("发现data-src，强制加载:", img.getAttribute('data-src'));
    ...
    function applyStaggeredAnimation(section) {
        // 限制一次动画的元素数量，只处理前15个元素
        const itemsToAnimate = section.querySelectorAll(".tech-card, .process-step, .case-study-item, .faq-item, .testimonial-item");
        const maxAnimatedItems = 15;
        
        // 使用 Array.from 转换为数组并限制数量
        Array.from(itemsToAnimate).slice(0, maxAnimatedItems).forEach((item, idx) => {
        ...
        // 其余元素直接显示，不应用动画
        Array.from(itemsToAnimate).slice(maxAnimatedItems).forEach(item => {
    ...
    */
    ...
    function lazyLoadImages() {
        ...
        // 确保所有图片容器都可见
        document.querySelectorAll('.certificate-card, .paper-card').forEach(card => {
            card.style.opacity = '1';
            card.style.animation = 'none';
        });
        
        // 特别为证书和论文图片设置样式
        document.querySelectorAll('.certificate-image, .paper-image').forEach(img => {
            img.style.opacity = '1';
            img.style.filter = 'none';
...
async function simulateModelResponse(userMessage) {
    ...
    if (lowerMessage.includes("创始人") || lowerMessage.includes("老板") || lowerMessage.includes("谁创建") || lowerMessage.includes("谁创办") || lowerMessage.includes("惠文清")) {
        return `我们团队的创始人是王文清。她创建了Hot科技创新服务平台，致力于为客户提供高质量的技术服务和解决方案。`;
        ...
        return `我们的数学建模服务根据需求不同有不同的价格。思路加图加代码是75元，完整论文加图加代码是185元，完全代写是500元。如果您有特殊需求，欢迎与我们沟通。`;
...
function initVisitorCounter() {
    ...
    function simulateRealTimeVisits(totalCount, todayCount) {
        // 增加时间间隔，减少更新频率，15-20秒一次
        const getRandomInterval = () => Math.floor(Math.random() * 5000) + 15000; // 15000-20000 ms (15-20 seconds)
        ...
        const incrementVisits = () => {
            ...
            // 降低今日访问数更新概率
            const shouldIncrementToday = Math.random() < 0.5;
            if (shouldIncrementToday) {
                const todayIncrement = Math.ceil(totalIncrement * 0.5);
...
window.onload = function() {
    ...
    document.querySelectorAll('img').forEach(img => {
        ...
        if (img.complete === false || img.naturalWidth === 0) {
            console.log("发现未完全加载的图片，重新加载:", img.src);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (bookingForm) {
        bookingForm.addEventListener('submit', function(e) {
            ...
            if (!name || !contact || !service || !details) {
                document.getElementById('booking-response').innerHTML = '<p class="error-message">请填写所有必填字段。</p>';
                return;
            }
            ...
            const response = document.getElementById('booking-response');
            response.innerHTML = '<p class="success-message">您的预约请求已安全提交，我们将尽快与您联系！</p>';
    ...
    formInputs.forEach(input => {
        // 防止浏览器自动填充敏感数据
        if (input.type === 'password' || input.id.includes('card')) {
...
function initializeSecurityFeatures() {
    ...
    window.addEventListener('click', function(event) {
        ...
        if (securityModal && event.target === securityModal) {
            securityModal.style.display = 'none';
        }
    });
...
function initMobileNavigation() {
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', function() {
            if (window.innerWidth <= 768 && document.body.classList.contains('mobile-nav-active')) {
                toggleMobileNav();
            }
        });
    });
...
// The initMobileNavigation function is already called in the existing DOMContentLoaded event listener above